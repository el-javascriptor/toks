...
function main() {
    ...
    if (canvas) {
        ...
        for (let i = 0; i < 500; i++) {
            const tok = new Tok(settings, random(0, settings.canvas_width), random(0, settings.canvas_height), new RGBColor(100, 150, 255), random(-0.5, 0.5), random(-0.5, 0.5));
            toks.push(tok);
        }
        update();
    }
